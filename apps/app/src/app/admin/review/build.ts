...
// Shared by the admin Review board and the manager (pod-scoped) one. Pass the
// pod's staff names to scope; null = the whole team (the admin view).
export function buildReviewProps(staffNames: ReadonlySet<string> | null = null) {
  const inScope = (staff: string | null) => staffNames === null || (staff !== null && staffNames.has(staff));
