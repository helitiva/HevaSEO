...
// Lane A cleanup — admin assigns an order to staff, gác③ (order state + assignee). assign_order is
// admin-gated + claims-derived; sets assignee_id and advances confirmed→assigned. Orders are SELECT-only
// for admin RLS, so the write goes through the fn.
