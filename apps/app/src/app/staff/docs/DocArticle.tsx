...
// The doc reader markup, shared by the staff and manager docs detail pages. The
// permission lookup (docForStaff / docForManager) happens in each page so an
...
export function DocArticle({ doc, backHref }: { doc: StaffDoc; backHref: string }) {
  ...
  return (
    <article className="mx-auto max-w-3xl">
      <Link href={backHref} className="mb-4 inline-flex items-center gap-1.5 text-sm font-semibold text-muted-foreground transition hover:text-foreground">
      ...
      {doc.resources.length > 0 && (
        <div className="mt-6">
          ...
          <ul className="space-y-2">
            {doc.resources.map((r) => (
              <li key={r.url}>
                <a href={r.url} target="_blank" rel="noopener noreferrer" className="kcard flex items-center gap-2 text-sm font-medium transition hover:border-primary/40">
      ...
      <p className="mt-8 flex items-center gap-1.5 rounded-lg bg-muted/50 px-3 py-2 text-[11px] text-muted-foreground">
        <i className="ph-bold ph-lock-simple" aria-hidden /> Published by admin · read-only.
      </p>
    </article>
  );
}
