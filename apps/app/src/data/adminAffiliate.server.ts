...
type AffRow = { id: string; code: string; status: string; tier: string; tier_pinned: boolean; joined_at: string | null; platform: string | null; niche: string | null; profiles: { name: string | null; email: string | null } | null };

export async function getAffiliates(): Promise<AdminAffiliate[]> {
  const supabase = await createClient();
  const [affs, refs, led, pays] = await Promise.all([
    supabase.from('affiliates').select('id, code, status, tier, tier_pinned, joined_at, platform, niche, profiles:user_id(name, email)').returns<AffRow[]>(),
  ...
  return (affs.data ?? []).map((a): AdminAffiliate => {
    ...
    return {
      ...
      platform: a.platform ?? '—', niche: a.niche ?? '—', email: a.profiles?.email ?? '', code: a.code,
