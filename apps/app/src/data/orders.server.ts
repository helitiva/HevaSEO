...
export async function getMyOrdersByProject(projectId: string): Promise<Order[]> {
  ...
  const { data, error } = await supabase
    .from('orders')
    .select('code, service, pkg, state, priority, value, deadline, created_at, delivered_at, customers(company, name), assignee:profiles!orders_assignee_id_fkey(name), order_details!inner(project, folder, title, site, project_id, proj:projects(domain))')
...
export async function getMyOrders(): Promise<Order[]> {
  ...
  const { data, error } = await supabase
    .from('orders')
    .select('code, service, pkg, state, priority, value, deadline, created_at, delivered_at, customers(company, name), assignee:profiles!orders_assignee_id_fkey(name), order_details(project, folder, title, site, proj:projects(domain))')
    .neq('state', 'canceled')
    .order('created_at', { ascending: false })
    .returns<MyOrderRow[]>();
