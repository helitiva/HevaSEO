...
export function ConversionFunnel({ stages }: { stages: FunnelStage[] }) {
  ...
  return (
    <section className="rounded-2xl border border-border bg-card p-5">
      <p className="flex items-center gap-2 text-sm font-semibold"><i className="ph-bold ph-funnel text-primary" aria-hidden /> Conversion funnel</p>
          ...
          return (
            <div key={s.key}>
              <div className="mb-1 flex items-center justify-between text-xs">
                <span className="flex items-center gap-1.5 font-medium"><i className={`ph-bold ${ICON[s.key]} text-muted-foreground`} aria-hidden /> {s.label}</span>
