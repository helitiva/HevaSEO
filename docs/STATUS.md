...
| **D** | staff_wallet + payroll (base/gig/commission/bonus−penalty, overrides, presets) | `wt-payroll` | 🟡 inc-D1 | claude | ②③ | E0d — **inc-D1 DONE** (`f311d54`): seed staff commission via real `post_staff_pay` (Mai's done/approved orders, 30% money-leak-safe) + `getMyStaffWallet()` reader (own staff_wallet+wallet_ledger, RLS money-blind to manager/customer). Verified e2e: Mai $77.40 / 3 entries / balance==SUM(ledger); manager+customer 0 rows. **NEXT inc-D2:** wire `/staff/finance` UI to real wallet (FinanceClient balance+ledger); then payout-request write (`payout_requests`) + penalties + manager_wallet override + admin payroll run. |
