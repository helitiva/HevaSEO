...
| **D** | staff_wallet + payroll (base/gig/commission/bonus−penalty, overrides, presets) | `wt-payroll` | 🟡 inc-D3 | claude | ②③ | E0d — **inc-D1** (`f311d54`): seed commission via real `post_staff_pay` + `getMyStaffWallet()` reader (RLS money-blind). **inc-D2** (`efda87f`): wired `/staff/finance` to real wallet (`realWallet` prop overrides balance/activity; demo/impersonation→mock fallback; fixed white-on-white wallet card). **inc-D3 DONE** (`0584eaa`): **payout request (write)** — `request_payout(amount, method)` DB fn (claims-derived + own-wallet-only → granted authenticated; atomic debit + `payout_requests` row, guards $50-min/sufficiency/method-ownership; pgTAP 0290) + `getMyStaffWallet` returns real methods+payouts + `requestPayoutAction` + FinanceClient wired (optimistic debit). **Verified e2e (Mai):** request $50 → $77.40→$27.40, 'requested' row w/ real PayPal method, invariant holds. 250 pgTAP + 378 app tests. **NEXT inc-D4:** admin approves/pays/rejects payout (reject=refund) + penalties write + manager_wallet override + admin payroll run. |
