...
| **D** | staff_wallet + payroll (base/gig/commission/bonus−penalty, overrides, presets) | `wt-payroll` | 🟡 inc-D2 | claude | ②③ | E0d — **inc-D1 DONE** (`f311d54`): seed staff commission via real `post_staff_pay` + `getMyStaffWallet()` reader (RLS money-blind). **inc-D2 DONE** (`efda87f`): wired `/staff/finance` UI to the real wallet — `getMyStaffWallet()` session-scoped + null-fallback (demo/impersonation→mock); `FinanceClient` `realWallet` prop overrides balance/available/activity-feed (penalties/payouts/payslips stay mock). Fixed pre-existing white-on-white wallet card (.kcard `background` shorthand killed the gradient). **Verified e2e (Mai signed in):** /staff/finance shows real $77.40 + 3 commission entries. 378 app tests. **NEXT inc-D3:** payout-request WRITE (`payout_requests` + a SECURITY DEFINER fn debiting the wallet) + penalties + manager_wallet override + admin payroll run. |
