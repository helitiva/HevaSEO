...
- **Follow-ups:** ✅ (1) RBAC role-shell gating in middleware (`2217531`); ✅ (3) regen database.types; ✅ (5) pgTAP for staff_details perf cols (`8af9fcb`). **Remaining:** (2) Playwright click-e2e for advance/cancel (browser env flaky — only SSR+node verified); (4) `profiles_same_tenant` over-broad read — **entangled**: tightening breaks the assignee-name embed in customer/order reads (a customer must read their order's staff name); needs a safe id+name+role view or denormalized assignee name → do carefully, not a quick fix.
...
- **NEXT ACTION:** Lane A **inc-3 (read swap) — IN PROGRESS.** Done: `getOrders()`/`getOrderById()` (RLS-scoped, `data/orders.server.ts`); `/admin/orders` (list + detail slide-over), `/admin/orders/[id]`, and `/admin` Command Center Needs-Attention all read REAL orders; `buildOrderDetailProps` takes a real `AdminOrder`. Seed: 6 customers + 11 orders.
  ...
  - **STEP 2 (next session) — DECISION LOCKED:** model `order_extra` as a **1:1 side table `order_details`** (order_id UNIQUE, brief jsonb, project, folder, addons jsonb, included) — own RLS per order, test-first via verify:db, seed for the seeded orders, then wire `buildOrderDetailProps` (drop mock fallbacks) + review + assignment. Then `qa_criteria` table, seed `deliverables`/`tickets`, migrate `/admin/review` + `/admin/assignment`. (~3-4 increments.) **STEP 3 = Lane B** (credit ledger UI + pricing engine → unlocks create-order-from-UI + Stripe).
