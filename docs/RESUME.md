...
- **NEXT ACTION:** Lane A **inc-3 (read swap) — IN PROGRESS.** Done: `getOrders()`/`getOrderById()` (RLS-scoped, `data/orders.server.ts`); `/admin/orders` (list + detail slide-over), `/admin/orders/[id]`, and `/admin` Command Center Needs-Attention all read REAL orders; `buildOrderDetailProps` takes a real `AdminOrder`. Seed: 6 customers + 11 orders.
  ...
  - **STEP 2 — IN PROGRESS:** ✅ **inc-5a** (`5bc0c28`) `order_details` 1:1 table (project/folder/brief/included) + RLS (admin/manager tenant, customer own, staff assigned via SECURITY DEFINER `order_assignee_id()`) + pgTAP `0250` + seed for 11 orders. ✅ **inc-5b** (`f398f9f`) `getOrderDetail()` + `buildOrderDetailProps(order, detail)` → `/admin/orders/[id]` + `/manager/orders/[id]` render REAL brief/project/included (verified). ✅ **inc-5c** (`32cc440`) `order_addons` (money) + RLS money-blind (admin/customer only; manager/staff get [] → no upsell block, no price leak) + wired into getOrderDetail/buildOrderDetailProps; verified admin-sees / manager-doesn't. 220 pgTAP. ✅ **inc-5d** (`6c72c6c`) `/admin/assignment` real — `getRules()` (assignment_rules) + buildAssignmentProps(roster, orders, rules); seeded 2 rules. ✅ **inc-5e** (`ad44e1c`) `/admin/review` real — deliverables += reviewed_at/review_note, `getDeliverables()`, buildReviewProps(staffNames, orders, deliverables, staff); seeded 5 deliverables. ✅ **inc-5f** (`3574b57`) slide-overs (OrdersExplorer + NeedsAttention) lazy-load real brief/addons via `useOrderDetail()` client hook (RLS-scoped; manager [] addons). **STEP 2 COMPLETE.** 222 pgTAP + 364 app tests. By-design leftovers (not debt): `qa_criteria`/SERVICE_INCLUDED kept as static config; manager review/assignment stay mock (no manager RLS on deliverables/rules — money/scope). **→ STEP 3 = Lane B — DONE.** ✅ **inc-B1** (`cb5d93e`) `/credit` reads real balance+ledger (`getMyCredit`, RLS; invoices+topup stay mock/Phase-2). ✅ **inc-B2** (`39816e6`) pure `computeOrderPrice(catalog, selection)` server pricing engine + 9 tests (`lib/orderPricing.ts`). ✅ **inc-B3** (`f1897a0`) create-order-from-UI: `placeOrderAction` (`app/(portal)/order.actions.ts`) — server-priced via `computeOrderPrice` (real tier, client total never trusted), `create_order` via **service-role client** (`lib/supabase/service.ts`, needs `SUPABASE_SERVICE_ROLE_KEY` at runtime — already in `apps/app/.env.local` local), debits credit, inserts order_details/order_addons (migration `20260630130000` grants service_role INSERT); `ServiceOrder.place()` wired, VIP display gated by real `isVip`. **Decided:** debit-now (cancelable-refund before staff accepts); VIP by real tier. **Verified e2e in browser:** jane placed $39 audit → 802→763, order AD-1040. **→ PHASE 2 — IN PROGRESS.** ✅ **inc-P1+P2** (`1deedb2`) simulated-Stripe top-up + real invoices: `invoices` table (migration `20260630140000`, RLS money-blind, pgTAP `0270`) + **payment-provider seam** `lib/payments/provider.ts` (MockProvider now → real Stripe later, same interface; `.99` amount = simulated decline) + `topUpAction` (`app/(portal)/credit.actions.ts`: validate → provider.charge → `topup` service-role fn → invoice row; client never self-credits) + `getMyCredit` returns real invoices + `CreditProvider` syncs from server props on `router.refresh()` (useEffect) so balance/history/invoices update live. **Verified e2e:** 2× $80 top-up → 802→962, sequential HD-2026-001/002, invariant holds. **Phase 2 remaining (deferred):** real Stripe keys/webhook (drop-in StripeProvider), quick-checkout 6-chốt for marketing site (materialize_order → create_order).
