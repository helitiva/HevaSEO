...
| K11 | **Một khuôn mẫu ledger, 3 instance** (eng-review F2) | 3 hệ tiền độc lập: `customer_credit`, `staff_wallet`, `affiliate_commission`. Mỗi cái bảng ledger riêng (append-only) + balance authoritative O(1) + payout flow — cùng khuôn K3, cùng độ atomic. Giữ rbac.ts capabilities (`finance.view`…) làm cổng cột. |
