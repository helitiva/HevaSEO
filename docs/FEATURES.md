...
| Staff wallet (staff self) | Staff see their own wallet (bonuses/adjustments net of penalties), pending balance, payout history. **Pay itself is payroll-truth:** the hero payroll card, earnings chart and YTD are derived by `payslipsToEarnings` (`data/staffPayroll.ts`) from the staffer's real `payroll_runs` (the same runs the Payslips tab shows), not mock earnings. Commission is paid on payroll, never minted into the wallet — so the two never double-count. | `/staff/finance` |
