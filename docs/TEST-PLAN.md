...
|---|-----|----------|----------|
| 1 | ✅ **RESOLVED — payroll is the source of truth.** `post_staff_pay` mints into `staff_wallet` but has zero app callers, so a real staffer's wallet commission was empty while `/staff/finance` showed mock earnings beside it. Rather than mint on delivery (which double-counts against the payroll accrual), the page now derives its pay figures from the staffer's real `payroll_runs` via `payslipsToEarnings` (`data/staffPayroll.ts`) — hero card + chart + YTD, consistent with the already-real Payslips tab. Wallet keeps bonuses/adjustments/penalties (copy fixed to say so); no minting → no double-count. Verified: Mai reads $1,029.60 take-home, $3,793.60 YTD from her real runs. | `data/staffPayroll.ts` + `staffPayroll.test.ts` (7) | DONE |
...
| 12 | ✅ **FIXED (staff + manager).** The created-account overlay was persisted to localStorage with a client-made id that never matched the real profile id, so after a reload the member showed **twice** — forever — once from the revalidated server roster and once from the stale overlay. The create actions already `revalidatePath`, so the overlay is only needed for the moment before that lands: made it **session-only (in-memory)**, discarded on reload when the real row has arrived. Verified: `/admin/staff` + `/admin/managers` render clean, console clear. (Affiliate self-register overlay is the intentional-mock case — see #11.) | `data/staffAccountsStore.ts`, `data/managerAccountsStore.ts` | DONE |
