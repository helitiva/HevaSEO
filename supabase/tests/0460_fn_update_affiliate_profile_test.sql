-- Lane E inc-E12: update_affiliate_profile — affiliate edits its OWN marketing metadata (claims-derived).
begin;
select plan(5);

select has_function('update_affiliate_profile', 'update_affiliate_profile() exists');
select ok(not has_function_privilege('anon', 'update_affiliate_profile(text,text,text)', 'execute'), 'anon CANNOT edit profile');
...
select throws_ok($$ select update_affiliate_profile('X','Y','Z') $$, 'NOT_AFFILIATE', 'customer cannot edit affiliate profile');

-- affiliate edits own → fields set
...
select update_affiliate_profile('YouTube', 'SEO', '120k subscribers');
